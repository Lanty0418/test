    ...
    try:
        ...
        try:
            if state_data and state_data.strip():
                ...
                llm_result = parsed_state.get("llm_result")
                slm_result = parsed_state.get("slm_result")
            else:
                llm_result = None
                slm_result = None
        ...
        if llm_result is None:
            ...
            llm_result = '{"analysis":"分析內容","classification":"完全錯誤"}'
        
        if slm_result is None:
            # 這裡你需要替換為實際從 state 取得的方式  
            # 例如：slm_result = agent.get_state("bert_result")
            slm_result = '{"score":"0.0046","classification":"錯誤"}'
...
LLM的結果為 state['bert_resfact_check_result_jsonult']
